...
// get company and related offers
...
export const softDeleteCompany = async(req , res , next)=>{
    ...
    if(company.offers.length){
        for (const offer of offers) {
            if (offer.Applications && offer.Applications.length) {
                for (const app of offer.Applications) {
                    app.deleteAt  = Date.now();
                    await app.save()
                }
                }
            offer.deletedAt = Date.now()
            await offer.save();
        }
    }
...
//save applications to excel sheet
export const saveAppToExcelSheet = async(req , res , next)=>{
    ...
    for (const offer of offers) {
        applications = [...applications , ...offer.Applications]
    }
