...
export const decodeToken = async(authorization, tokenType = tokenTypes.ACCESS , next) => {
    ...
    if (!barer || !token) 
        return next( new AppError('please send token', StatusCodes.BAD_REQUEST) );
    ...
    if (!signature) 
        return next( new AppError('Token signature is missing', StatusCodes.BAD_REQUEST) );
    ...
    try {
        ...
        if (!user) 
            return next( new AppError('Invalid token', StatusCodes.BAD_REQUEST) );

        return { user  , accessSignature};
    } catch (error) {
        return next( new AppError('Token verification failed', StatusCodes.BAD_REQUEST) );
    }
};
